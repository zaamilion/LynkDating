...
FRIENDS_SERVICE_URL = (
    f"http://{settings.FRIEND_SERVICE_HOST}:{settings.FRIEND_SERVICE_PORT}"
)
...
async def get_friends_requests_receiver_list(cookie) -> int:
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(
                f"{FRIENDS_SERVICE_URL}/friend_requests_receiver_list", cookies=cookie
            )
            response.raise_for_status()
